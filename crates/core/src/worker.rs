...
//! Linux's flat ELF namespace offers even less protection. Nor is Apple's silence the
//! norm: `rust-lld` and MSVC's `link.exe` both refuse the link outright, and the
//! `build.rs` of this crate and of `app` are what ask them for Apple's behaviour.
//!
